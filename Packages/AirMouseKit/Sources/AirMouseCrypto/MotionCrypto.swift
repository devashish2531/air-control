...
///
/// This module does not reference `AirMouseProtocol.MotionPayload` (that module is being written
/// concurrently — see module header); `seal`/`open` operate on the raw 16-byte plaintext buffer.
/// Integration note: `AirMouseProtocol.MotionPayload.pack(into:)`/`unpack(from:)` should produce/consume
...
public enum MotionCrypto: Sendable {
    ...
    /// (host session logic in `AirMouseCore`, not yet written) owns the actual sessionID → key table.
