...
///   (`AirMouseCore`, not yet written), not by this pure value type; `PairingSecret` only models the
...
public struct PairingSecret: Sendable, Equatable {
    ...
    /// `airmouse://pair?...` QR URL (spec §3.2.2 sequence diagram: "QR = airmouse://pair?…"). The QR
    /// payload's other fields (addresses, host FP, etc.) and its full URL grammar are owned by
    /// `AirMouseProtocol.QRPayload` (arch §3.1) — this module only encodes/decodes the secret itself.
