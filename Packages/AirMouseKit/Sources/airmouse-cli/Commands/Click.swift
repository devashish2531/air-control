...
// `airmouse-cli click [--button left|right|middle] [--double]` — sends a `click` message (spec
...
struct ClickCommand: AsyncParsableCommand {
    ...
    func run() async throws {
        ...
        try await session.sendClick(AirMouseProtocol.Click(button: button, action: .tap, count: 1, modifiers: []))
        if double {
            try await session.sendClick(AirMouseProtocol.Click(button: button, action: .tap, count: 2, modifiers: []))
        }
