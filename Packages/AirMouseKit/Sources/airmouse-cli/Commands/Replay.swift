...
// `airmouse-cli replay <file.jsonl>` — replays recorded events with their original timing. Each line
    ...
    func run() async throws {
        ...
        for event in events {
            ...
            case "scroll":
                ...
                try await session.sendMotion(payload)
            case "click":
                try await session.sendClick(AirMouseProtocol.Click(button: .left, action: .tap, count: 1, modifiers: []))
            default:
                continue
            }
            sent += 1
