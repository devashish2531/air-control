...
/// The control (TCP/TLS) channel abstraction `AirMouseCore` drives; implemented by the apps and
/// `airmouse-cli` with `Network.framework` (arch §3.1 rule 4/6: "Core never imports `Network`").
///
/// A `ControlChannel` carries already-framed bytes (the output of
/// `AirMouseProtocol.FrameEncoder`/input to `FrameDecoder`, spec §3.4.1) — `AirMouseCore` owns
...
/// apps with `Network.framework`; `AirMouseCore` only seals/opens/coalesces payloads and hands
...
/// `ControlChannel` is — `AirMouseCore` never imports `Network`, so it cannot open the UDP
