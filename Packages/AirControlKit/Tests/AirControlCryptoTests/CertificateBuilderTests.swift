...
@Suite struct CertificateBuilderTests {
    @Test func buildsAndSelfSigns() throws {
        ...
        let output = try CertificateBuilder.makeSelfSigned(privateKey: key, commonName: "AirControl Host test-host")
    ...
    @Test func parsesBackAndVerifiesSelfSignature() throws {
        let key = P256.Signing.PrivateKey()
        let output = try CertificateBuilder.makeSelfSigned(privateKey: key, commonName: "AirControl Client test-client")
    ...
    @Test func derRoundTripIsByteExact() throws {
        let key = P256.Signing.PrivateKey()
        let output = try CertificateBuilder.makeSelfSigned(privateKey: key, commonName: "AirControl Host reround")
        let parsed = try CertificateBuilder.parse(der: output.der)
    ...
    @Test func fingerprintMatchesIndependentSHA256() throws {
        let key = P256.Signing.PrivateKey()
        let output = try CertificateBuilder.makeSelfSigned(privateKey: key, commonName: "AirControl Host fp-check")
    ...
    @Test func defaultValidityIsApproximatelyTenYears() throws {
        ...
        let output = try CertificateBuilder.makeSelfSigned(
            privateKey: key,
            commonName: "AirControl Host validity",
    ...
    @Test func differentCommonNamesProduceDifferentCertificates() throws {
        let key = P256.Signing.PrivateKey()
        let a = try CertificateBuilder.makeSelfSigned(privateKey: key, commonName: "AirControl Host A")
        let b = try CertificateBuilder.makeSelfSigned(privateKey: key, commonName: "AirControl Host B")
        #expect(a.der != b.der)
