...
@Suite struct IdentityFactoryEphemeralTests {
    @Test func buildsAUsableSecIdentity() throws {
        let identity = try IdentityFactory.makeEphemeralIdentity(commonName: "AirControl Host ephemeral-1")
    ...
    @Test func secIdentityTFallbackIsNonNil() throws {
        ...
        let identity = try IdentityFactory.makeEphemeralIdentity(commonName: "AirControl Host ephemeral-2")
        #expect(identity.secIdentityT != nil)
    }

    @Test func twoEphemeralIdentitiesHaveDifferentFingerprints() throws {
        let a = try IdentityFactory.makeEphemeralIdentity(commonName: "AirControl Host ephemeral-A")
        let b = try IdentityFactory.makeEphemeralIdentity(commonName: "AirControl Host ephemeral-B")
...
extension IdentityTierRoundTripTests {
    @Test func persistedIdentityRoundTrip() throws {
        let label = "AirControlCryptoTests.identity.\(UUID().uuidString)"
        ...
        do {
            let identity = try IdentityFactory.makeIdentity(
                ...
                commonName: "AirControl Host keychain-test \(label)",
                label: label,
