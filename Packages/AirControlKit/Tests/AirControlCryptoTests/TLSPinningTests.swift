...
@Suite struct TLSPinningTests {
    ...
    @Test func extractsLeafDERFromSingleCertificateTrust() throws {
        ...
        let output = try CertificateBuilder.makeSelfSigned(privateKey: key, commonName: "AirControl Host trust-test")
    ...
    @Test func computesMatchingFingerprint() throws {
        let key = P256.Signing.PrivateKey()
        let output = try CertificateBuilder.makeSelfSigned(privateKey: key, commonName: "AirControl Host fp-trust-test")
        let trust = try makeSecTrust(der: output.der)
