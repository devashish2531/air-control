...
/// > `AirControlCrypto.IdentityFactory`.
///
...
///    `#if SEC_OS_OSX`); it does not exist on iOS at all, so `AirControlCrypto` cannot build for the iOS
...
public enum IdentityFactory: Sendable {
    ...
    ///
    /// - Parameters:
    ///   - commonName: certificate CN, e.g. `"AirControl Host <hostID b64u>"` (spec §3.2.1).
    ...
    public static func makeEphemeralIdentity(
        commonName: String,
        label: String = "AirControlCrypto.ephemeral.\(UUID().uuidString)"
    ) throws -> GeneratedIdentity {
