...
// here so `AirControlProtocol` (Foundation-only, no Carbon import) and every other module — including
// the iOS app, which cannot import Carbon at all — can reference them by name instead of hardcoding
// magic numbers (CLAUDE.md: "every wire constant comes from AirControlProtocol constants, never
