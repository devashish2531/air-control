...
// The control-channel (TCP/TLS) `ControlChannel` implementation for `aircontrol-cli` — the one package
...
final class CLIControlChannel: ControlChannel, @unchecked Sendable {
    ...
    static func connect(host: String, port: Int, identity: SecIdentity, pinnedFingerprint: Fingerprint) async throws -> CLIControlChannel {
        ...
        let tlsQueue = DispatchQueue(label: "com.aircontrol.cli.tls")
    ...
    /// spec §3.2.3: the TLS exporter for pairing-proof binding, label `EXPORTER-aircontrol-pairing-v1`,
