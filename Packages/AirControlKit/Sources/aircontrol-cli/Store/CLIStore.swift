...
// Persists `aircontrol-cli`'s own client identity and the hosts it has paired with, as JSON under
// `~/.aircontrol-cli/` (0600 file perms, 0700 directory) — deliberately *not* the Keychain-backed
// `AirControlCrypto.IdentityStore`/`IdentityFactory.makeIdentity` path (spec §7.1 Path A), because a
...
// goes through `AirControlCrypto.CertificateBuilder` (spec §3.2.1: self-signed P-256, 10-year validity,
...
// (`AirControlCrypto.IdentityFactory.makeEphemeralIdentity`, which is deliberately never persisted).
...
/// One `aircontrol-cli` client identity, persisted as raw key material + certificate DER.
...
enum CLIStore {
    ...
    static var directoryURL: URL {
        FileManager.default.homeDirectoryForCurrentUser.appendingPathComponent(".aircontrol-cli", isDirectory: true)
    }
    ...
    private static func makeAndPersistIdentity() throws -> IdentityRecord {
        ...
        let commonName = "AirControl CLI Client \(Data(SystemRandom.bytes(16)).b64u)"
    ...
    /// + `SecIdentityCreate` recipe `AirControlCrypto.IdentityFactory` uses for its own ephemeral path.
...
/// Minimal `SecRandomCopyBytes` wrapper so this file needs no `AirControlCrypto` internal (its
