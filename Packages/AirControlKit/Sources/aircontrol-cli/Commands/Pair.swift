// Commands/Pair.swift
// `aircontrol-cli pair <aircontrol://pair?...>` — first-time pairing (spec §3.2.2): parse the QR/manual
...
struct Pair: AsyncParsableCommand {
    static let configuration = CommandConfiguration(
        abstract: "Pair with a host using a scanned/typed aircontrol://pair URL."
    )

    @Argument(help: "The aircontrol://pair?... URL (from the host's pairing QR/window).")
    var url: String
