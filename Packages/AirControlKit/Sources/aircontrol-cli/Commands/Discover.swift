...
// `aircontrol-cli discover` — browses `_aircontrol._tcp` (spec §3.1.1: "the client browses only
// `_aircontrol._tcp`... `includePeerToPeer = false`") and prints each result's TXT record (spec §3.1.2).
...
struct Discover: AsyncParsableCommand {
    static let configuration = CommandConfiguration(
        abstract: "Browse the local network for Air Control hosts (Bonjour _aircontrol._tcp)."
    )
    ...
    func run() async throws {
        ...
        if json {
            ...
            print("No Air Control hosts found in \(timeout)s.")
        } else {
