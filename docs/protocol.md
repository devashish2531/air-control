# Air Control wire protocol (contributor reference)
...
Types live in the SwiftPM target `AirControlProtocol` (`Packages/AirControlKit/Sources/AirControlProtocol`)
— decisions Addendum C1 renamed this from the spec's original `AirControlWire`; you may still see
...
The host registers Bonjour services **`_aircontrol._tcp`** (control) and **`_aircontrol._udp`**
(motion) with the same instance name and TXT record; the client browses only `_aircontrol._tcp`.
...
Vectors live in `Packages/AirControlKit/Tests/*/Vectors/*.json`, generated by
