# Air Control — Implementation Log
...
Repo skeleton: `Packages/AirControlKit` with stub targets (Protocol, Crypto, Filters, Core, aircontrol-cli) and Swift Testing smoke tests;
`apps/AirControl-iOS` and `apps/AirControl-Mac` XcodeGen specs from arch §2.4/§2.5 (Sparkle deferred); `Config/*.xcconfig`; `Makefile`;
...
|---|---|---|
| Protocol | `Sources/AirControlProtocol/**` except `Keycodes/` | §3.0–3.7, §5.5 model, §6.1–6.2, §11.1, §11.3 |
| Crypto | `Sources/AirControlCrypto/**` | §3.2, §3.5, §6.4, §7.3, arch §7 |
| Filters | `Sources/AirControlFilters/**` | §4.2, §4.3, §5.3–5.4, §6.3 |
| Keycodes | `AirControlProtocol/Keycodes/`, Mac `Services/KeycodeMapper/` | §4.4, §11.2 |
...
iOS ConnectionManager/NetworkTransport/Pairing/Devices, iOS Touchpad + MotionPublisher, iOS Gyro, iOS Keyboard, iOS Remote/Macros, aircontrol-cli + loopback harness.
...
- `PRODUCT_NAME` changed from `Air Control` to `AirControl` in both `project.yml` files (the space broke `TEST_HOST` for unit-test bundles). `CFBundleDisplayName` stays "Air Control"; `PRODUCT_MODULE_NAME = Air_Control` keeps existing `@testable import Air_Control` working.
...
- Interrupted by an API usage limit and resumed from saved context: AirControlCore (all files present, finishing E2E loopback tests), iOS Touchpad/MotionPublisher (build green, tests pending), iOS Gyro (verification pending), AirControlCrypto (final report pending).
...
- Pending launch (blocked on Core's session API): Mac HostServer/PairingService/TrustStore, iOS ConnectionManager/Transport/Pairing/Devices, aircontrol-cli + loopback integration tests.
...
- App targets: `PRODUCT_NAME: AirControl` (no space), `PRODUCT_MODULE_NAME: Air_Control`, `ENABLE_DEBUG_DYLIB: NO` (otherwise the executable is a 40 KB stub and the test bundle cannot link app symbols).
...
- AirControlCore: 141 unit tests green in 14 suites; the E2E loopback suite deadlocked (hung `swift test` killed after 50 min) and is being fixed by its agent with a per-test timeout.
...
- Launched: Mac HostServer/SessionManager/PairingService/TrustStore/PairingWindow; iOS ConnectionManager/NetworkTransport/Pairing/Devices; `aircontrol-cli` + Mac loopback integration tests. Loopback contract: `--loopback` prints `AIRCONTROL_TCP_PORT`, `AIRCONTROL_UDP_PORT`, `AIRCONTROL_PAIR_URL` lines and writes injected events as JSON lines to `$AIRCONTROL_LOOPBACK_LOG`.
...
In flight: (a) Core E2E fix, (b) Mac HostServer/SessionManager/PairingService/TrustStore/PairingWindow, (c) iOS ConnectionManager/NetworkTransport/Pairing/Devices (fresh agent), (d) aircontrol-cli + Mac loopback integration tests.
To resume: 1) confirm each in-flight item has files on disk and builds (`make kit-test`, `make ios-build`, `make mac-build`); 2) re-launch anything incomplete with a compact prompt (inline the API signatures; instruct "one file per tool call, build every 3 files, no background monitors") because large-context agents stall; 3) integration agent: swap Placeholder* services in `apps/AirControl-Mac/Sources/App/AppEnvironment.swift` and NoOp* services in `apps/AirControl-iOS/Sources/App/AppEnvironment.swift` for the real factories (`HostFeature.make`, `MacroFeature.make`, `ConnectionFeature.make`, `TouchpadFeature.make`, `KeyboardFeature.makeBridge`, `RemoteFeature.make`, `MacrosFeature.make`, `AirPointerFeature.make`), update RootTabView call sites, then `make build && make test`; 4) run the loopback E2E (helper `--loopback` + `aircontrol-cli pair/move/click/type`); 5) append results here and in `docs/00-decisions.md` errata (spec §6.4 replay vector inconsistency; `scrollWheelEventPointDeltaAxis` finding); 6) the tree is uncommitted since the bootstrap commit — ask the owner before committing.
...
- aircontrol-cli: 11 subcommands (discover, pair, connect, move, click, scroll, type, key, media, bench, replay); identity + trusted hosts in `~/.aircontrol-cli/` (0600). Mac loopback integration suite written; it skips until the helper wires `HostServer.start()`.
...
|---|---|
| AirControlKit (Protocol, Crypto, Filters, Core, aircontrol-cli) | `swift test`: 520 tests / 60 suites green |
...
- CI: kit and lint green; iOS and Mac jobs failed within 2 min because `AIRCONTROL_WARNINGS_AS_ERRORS=YES` promoted two Swift 6 concurrency warnings (`DisplayTopology.swift`) to errors. Gate relaxed to `NO` in `ci.yml` until the app targets are warning-clean; a cleanup pass is running, after which the gate returns to `YES`.
- Accessibility: the helper is ad-hoc signed, so every rebuild invalidates the TCC grant while System Settings still shows it enabled. Added `make mac-run` (build → `tccutil reset Accessibility com.aircontrol.helper` → launch); re-grant when prompted. Permanent fix is a stable Apple Development identity in `Config/Local.xcconfig` (decisions A10).
- Warning cleanup: 6 Mac + 4 iOS Swift warnings fixed (`@ObservationIgnored` on `nonisolated(unsafe)` stored properties under `@Observable`, `MainActor.assumeIsolated` in main-queue callbacks, deprecated `String(cString:)`, non-Sendable captures). Both apps build clean with `AIRCONTROL_WARNINGS_AS_ERRORS=YES`; CI gate restored to `YES`. `ALWAYS_SEARCH_USER_PATHS=NO` silences the headermap notice.
...
- `com.aircontrol.app` is not registrable on the owner's team; bundle IDs are now `$(IOS_BUNDLE_ID)` / `$(MAC_BUNDLE_ID)` from `Config/Base.xcconfig`, overridden in git-ignored `Local.xcconfig` (`com.aircontrolios.app` on this machine). Command-line device build + `devicectl` install works; `make ios-run` added. Mac helper signed post-build via `make mac-run` (Xcode's CLI provisioning cannot sign Mac package products).
...
- `aircontrol-cli discover` returning nothing from Terminal is the CLI process lacking its own Local Network permission on macOS 26 (TXT record verified well-formed with `dns-sd -L`).

### On-device regression campaign (2026-09-05 16:30–17:30 IST)
Test rig: `AirControl --print-pair-url` (dev flag, normal networking) + iOS DEBUG launch hook `AIRCONTROL_PAIR_URL` + `PairingUITests` run on the real iPhone via `xcodebuild test-without-building -destination id=<UDID>` with `TEST_RUNNER_AIRCONTROL_PAIR_URL=<url>`; `AIRCONTROL_PAIR_SECRET_TTL=900` (DEBUG) covers the multi-minute install cycle; host logs via `log stream --predicate 'process == "AirControl" AND subsystem == "com.aircontrol.helper"'` (`log show` does not persist this helper's logs).
Root causes found and fixed, in the order they were hit:
1. **Pairing "expired" on the phone = device limit.** 19 of 20 trust slots were `AirControlHelperIntegrationTests` identities: every loopback/integration run paired into the developer's real `TrustedDevices.json`. Fixed with `AIRCONTROL_DATA_DIR` (DocumentStore override), auto-set to a temp dir for `--loopback` and by the integration harness. The polluted store was moved to `_removed-*/`.
...
- Mac helper's host TLS identity now picks a Keychain tier at runtime: data-protection keychain (no ACL prompts) when the running binary carries an application identifier, else a legacy-keychain item with a no-prompt ACL (`SecAccessCreate`/`SecACLSetContents`, any-application), else an ephemeral in-memory identity. Fixes recurring "AirControl wants to use your confidential information…" prompts. `AirControlCryptoTests` 98/98 green (round-trip + migration-decision tests).
...
- **Reverted**: an attempt to re-sign the helper with `com.apple.application-identifier` + `keychain-access-groups` in `make mac-run` broke launch entirely (`Launchd job spawn failed`, RBSRequestErrorDomain 5) because those are provisioning-profile-gated entitlements plain `codesign` cannot satisfy on this machine. `make mac-run` signs with `AirControlHelper.entitlements` only, as before.
...
### Real-device pairing fixes (2026-09-05 evening)
- TLS handshake failed from the real iPhone (Mac: `accepted → TLS handshake failed`; phone: `signing-failed … -50 … ECPublicKey`): the client key was created with `kSecAttrIsPermanent`/label at the top level, so the public key was persisted under the same label and later identity lookups joined the certificate to the public key. Fixed in `IdentityFactory` (attributes under `kSecPrivateKeyAttrs`, public key not persisted), `IdentityStore` (signing probe), and `ConnectionManager+Environment` (re-mint an unusable identity).
- Re-pairing an already trusted device hung then failed as "internal": the host auto-authenticated any known peer (ignoring `pairing:true`) while the client waited for `pairChallenge`. Host now runs the pairing flow for known peers when the window is open (updating the existing trust record) and replies `pairing.alreadyTrusted` otherwise; the client accepts a bare `helloAck` during pairing; new `CoreError.protocolMismatch` replaces internal failures; iOS maps these to readable copy. Core: 151 tests green; live CLI double-pair verified.
- Keychain incident: a subagent's cleanup wiped the owner's login-keychain certificates and keys (passwords intact); certificate recreated by the owner; hard rules added to CLAUDE.md.
- `make ios-run` now accepts devices in `available` state (Wi‑Fi) and parses the UDID by pattern.

### UI pass and motion investigation (2026-09-06)
- Touchpad: centred status pill removed; right-edge scroll strip (48 pt) feeding the existing scroll intents; 64 pt left/right click buttons via bottom safe-area inset. Toolbar pill: fixed-width capsule with calibrated offset (iOS 26 centres un-grouped leading items on an icon-sized slot). Keyboard: adaptive rows that wrap instead of overflowing, fixed 44 pt media circles, hide/show keyboard control, Done accessory, interactive dismiss, keyboard-height inset. ConnectionManager: debounced (1 s) generation-tagged suspend fixes the stuck "Suspended" pill.
- Motion: the phone pipeline touch → TouchpadUIView → TouchpadController → MotionPublisher.sentDatagramCount is verified working (DEBUG counters + TouchpadMotionPipelineUITests). CLI motion moves the Mac cursor. Remaining gap is between MotionDatagramSink.sendMotion and the helper: ClientSession.sendMotion silently throws when no session key / no datagram channel, and probe-loss accounting may never trigger TCP fallback when the UDP channel is absent.

### Motion fix and UI revamp wave (2026-09-06 afternoon)
- Motion not reaching the Mac from the phone: `ClientSession.sendProbe()` silently no-op'd when the UDP channel was nil or threw, so `ProbeController` never saw losses and TCP fallback never engaged; `NWDatagramChannel.connect` (iOS) waited for `.ready`, which UDP on real Wi‑Fi may never report; `ConnectionMotionSink` swallowed every error. Fixed all three; failures now surface in Diagnostics and the DEBUG label (`sendMotion failures: kind×N`). Loopback E2E UI test (`testSwipesReachHelperLoopbackLog`) recorded 260 `mouseMoved` lines for 20 drags. Commit `5ac88c5`.
- UI revamp planned in `docs/08-ui-revamp.md` and implemented by four parallel Sonnet agents: iOS shell (`ConnectionStatusDot` replaces the toolbar pill; touchpad card surface; `TabSwitcher` env; appearance applied at root), keyboard tab (trailing-only toolbar, one sectioned scrolling key bar, `KeyboardAccessoryBar` above the system keyboard), theme/onboarding (Appearance picker, colour audit, onboarding bottom button stack, Remote/Air Pointer restyle), Mac app (LSUIElement off, "Air Control" display name, main window with Overview/Devices/Macros/Diagnostics/Settings, `MainWindowRouter`, Show in Dock). Commits `2066be5`, `638146d`.
- Operational: `make mac-test` cannot run while the built helper is running from the same DerivedData path (xcodebuild copies test frameworks into the live bundle and fails). Run tests with `-derivedDataPath` elsewhere, detached with `nohup`, when the helper is up. `codesign` prompts for the keychain key on every run until the owner clicks "Always Allow".
- Open: owner verification of motion on device after `5ac88c5`; Mac Overview lacks "connected since" (host service exposes no timestamp); iOS display name still "Air Control".
