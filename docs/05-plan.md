...
| **M2** | Walking skeleton | Phone discovers Mac, connects over TLS (trust-all stub), a tap clicks and a drag moves the cursor via TCP | First cursor move end-to-end video; loopback integration test passes in CI; `aircontrol-cli connect/click/move` works; `docs/protocol.md` extracted | 9 d | M1 |
...
| **M6** | Gyro air-pointer | Gravity-aware mapping, One-Euro, dead zone, bias/auto-freeze, clutch, recenter, calibration | 0 px creep resting 10 min; < 20 px/min creep in a 45-min hold; flat and upright grips move the same way; p95 ≤ 20 ms | 5 d | M4 |
...
| M0-05 | Create `Packages/AirControlKit/Package.swift` exactly as arch §2.2 with four empty library targets, four test targets (one placeholder test each), and an `aircontrol-cli` stub printing its version; root `Package.swift` shim | Kit | arch §2.2 | 3 | `swift build` and `swift test` succeed from the repo root with language mode 6 and strict concurrency; `swift run aircontrol-cli --version` prints |
| M0-06 | `apps/AirControl-iOS/project.yml` per arch §2.4 with a "Hello" `TabView`; Info.plist keys (`NSLocalNetworkUsageDescription`, `NSBonjourServices`, `NSCameraUsageDescription`, `NSMotionUsageDescription`, `aircontrol` URL scheme) | iOS | §2.3 | 3 | `xcodegen generate` then `xcodebuild -scheme AirControl -destination 'platform=iOS Simulator,name=iPhone 16' build CODE_SIGNING_ALLOWED=NO` succeeds; plist keys visible in the built app |
| M0-07 | `apps/AirControl-Mac/project.yml` per arch §2.5 with a `MenuBarExtra` "Hello", `LSUIElement`, hardened runtime, universal `ARCHS`, entitlement file; `Config/Base.xcconfig` with `#include? "Local.xcconfig"`; stable Apple Development signing in Debug | Mac | §2.3, A10 | 3 | Built app shows only a menu-bar icon (no Dock icon); `codesign -dv --verbose=2` shows the Apple Development identity and `runtime` flag; rebuilding twice keeps the same designated requirement |
| M0-08 | `.swiftlint.yml` + `.swiftformat` with custom rules: ban `CGEventTapCreate`/`CGEvent.tapCreate`/`IOHIDManager`, ban `print(` outside `aircontrol-cli`, ban `usleep`/`Thread.sleep`, flag `Logger` interpolations naming secrets without a privacy annotation | Repo | arch §7.5, ADR-009 | 2 | A test file containing `CGEvent.tapCreate(` fails `swiftlint --strict`; `swiftformat --lint .` passes on the skeleton |
...
| M2-03 | `TXTRecordModel` (v, n, id, fp, m, tp, up; ≤ 400 B) and `QRPayload` parse/format (`aircontrol://pair?…`, ≤ 512 B, address ordering preserved) | Kit/Protocol | §3.1.2, §3.1.3 | 4 | §6.4 QR vectors incl. malformed cases (missing `s`, 513 B, bracketed IPv6) pass |
| M2-04 | `AirControlCore` transport protocols (`ControlTransport`, `DatagramTransport`, `ServiceBrowser`, `ServiceAdvertiser`), `HostSessionStateMachine` (pending → authenticated → stale → closed) and `ConnectionStateMachine` reducer with Idle/Browsing/Connecting/Connected/Reconnecting/Failed | Kit/Core | §4.5.1, arch §3.1 | 8 | Reducer tests for the implemented edges; `MockTransport` scripted test drives Idle → Connected |
| M2-05 | Mac `HostServer` actor on the `net` `DispatchSerialQueue`: `NWListener` TLS 1.3 with an ephemeral `IdentityFactory` identity, **temporary trust-all verify block** (`// TODO M3`), Bonjour advertise with TXT, `serviceRegistrationUpdateHandler` name capture, port-busy ephemeral fallback | Mac | §3.1.1, §5.1.5 | 6 | `dns-sd -B _aircontrol._tcp` shows the service with the TXT keys; a second instance falls back to an ephemeral port and advertises it |
...
| M2-13 | `--loopback [--port N] [--identity test]` launch mode (127.0.0.1, in-memory identity, `RecordingInjector`, JSON control socket exposing the event log); `AirControlHelperIntegrationTests` target with the first test: connect → `hello`/`helloAck` → `click` recorded | Mac, CI | §10.2 | 6 | `xcodebuild test` runs the integration test in CI without any TCC grant |
| M2-14 | `aircontrol-cli connect --host <addr or name>`, `click`, `move --dx --dy --hz --seconds` over the TCP path; shares `NWControlTransport` source with the iOS app via a symlinked/shared source folder or a small `AirControlNetwork` module inside the CLI | CLI | §10.2 | 4 | `swift run aircontrol-cli connect --host 127.0.0.1:47800 --insecure` moves the cursor on a dev Mac |
...
| M3-02 | `IdentityFactory` production implementation per M1 decisions (Path A/B selection, Keychain labels `AirControl Host/Client Identity`, load-or-create, "Reset identity") for both platforms | Kit/Crypto, iOS, Mac | §3.2.1, arch §7.1 | 8 | On-device iOS test creates and loads an identity; macOS unit test does the same; identity survives relaunch; reset regenerates |
| M3-03 | Mac `TrustStore` actor: `kSecClassCertificate` items labelled `AirControl Trusted Client <clientID>` + `TrustedDevices.json` (`trusted-devices/1`), records dropped when no cert matches, 20-device cap, revoke (delete cert, mark record) | Mac | §3.2.4, §5.6 | 6 | Unit tests with a temp keychain: add/lookup/revoke; JSON without cert is pruned on load |
...
| M3-08 | iOS `KeychainStore` (identity via `IdentityFactory`, trusted host certs labelled `AirControl Trusted Host <hostID>`) + `DocumentStore` actor with `TrustedHosts.json` (`trusted-hosts/1`, atomic, file protection) and `TrustedHostRecord` | iOS | §3.2.4, §4.7 | 6 | Unit tests on simulator (Path B); Forget deletes cert + record + macro cache |
| M3-09 | iOS Scan QR screen: `DataScannerViewController` with `AVCaptureSession` fallback, reticle, stop on `aircontrol://pair?`, haptic, torch, "Paste pairing link" field with validation, camera-denied E-CAMERA | iOS | §4.1.2, §4.5.5 | 6 | Scans a QR in < 1 s; pasted URL pairs; denial shows E-CAMERA with Settings deep link |
...
| M4-02 | `SessionKeys.derive` (HKDF-SHA256, salt = sessionID LE, infos `aircontrol-udp-c2h-v1`/`h2c-v1`) and `MotionCrypto.seal/open` (nonce `0000 ‖ counter LE`, AAD = header, exactly 44 B) with `OpenFailure` side channel | Kit/Crypto | §3.5.1, §3.5.3 | 5 | §6.4 AEAD vector; flipping each of the 44 byte positions fails; wrong sessionID → nil; counter 2³² boundary stops sending |
| M4-03 | **Implement the RFC 6479 replay window in `AirControlCrypto`** (`highest`, 64-bit bitmap; accept newer with saturating shift; reject ≥ 64 behind; reject seen) **with tests for wraparound** (jump > 64 clears the bitmap; `highest − counter == 64` rejected) | Kit/Crypto | §3.5.4 | 4 | §6.4 sequence → `[A,A,A,R,A,R,A,R,A,A,R]`; 10⁶ random counters vs a `Set` oracle agree |
...
| M4-25 | **Performance gate run** (spec §10.4): `aircontrol-cli bench` vs `--bench`; `xctrace` inter-event jitter p95 ≤ 12 ms; 240 fps camera ground truth on a 120 Hz iPhone with the Mac at 60 and 120 Hz; tune `fingerCountSettle`/tap thresholds (R-5); fix hot-path regressions | iOS, Mac, CLI | §8.1, §10.4 | 8 | p50 ≤ 12 ms, p95 ≤ 20 ms (+ display half-frame) to post on the reference setup; numbers recorded in `docs/perf/M4.md` |
...
### M6 — Gyro air-pointer mode (42 h ≈ 5 d)
...
| M6-08 | Hide the Air Pointer tab when `isDeviceMotionAvailable == false`; Settings › Gyro shows E-GYRO-NONE | iOS | §4.1.5, §9 | 1 | Simulator (no gyro) hides the tab and shows the explanation |
...
| M8-10 | iOS onboarding: 3 pages (value prop; install helper with QR to Releases + copyable `brew install --cask air-control`; Local Network pre-prompt), Skip on every page, denial screen E-LOCALNET with re-check on return, camera denial path | iOS | §4.1.1, §4.5.5 | 6 | Fresh install → paired in one sitting; denial → Settings → return re-checks automatically |
...
| M8-12 | Error presentation: complete E-* mapping table for both apps (titles, messages, recovery actions of §9), fallback "Connection problem / <code>" | iOS, Mac | §9 | 4 | Snapshot test: every `AirControlError` case maps to an E-* id; unmapped peer code shows the fallback |
...
| M9-04 | Homebrew project tap repo `homebrew-aircontrol` with `Casks/air-control.rb`; `scripts/release/bump-cask.sh` opens a PR with the new version/sha256 via `gh`; note the homebrew-cask notability path (R-14) | CI, Repo | NFR-OSS-005, R-14 | 4 | `brew install --cask <owner>/aircontrol/air-control` installs the notarized app; the release job opens the bump PR |
...
| M9-08 | `nightly.yml`: fuzzers with `AIRCONTROL_FUZZ_ITERATIONS=1000000`, 30 min loopback soak with memory delta assertion | CI | §10.1, §10.4 | 3 | Nightly green for 7 consecutive nights before release |
...
| M9-11 | Name/trademark check (USPTO, EUIPO, WIPO) and App Store name availability; pick the final name (candidates Waft, Glidepad, Hover Remote); rename codename in bundle IDs, `project.yml` ×2, cask, service type decision (R-10: keep `_aircontrol` or register a distinct type), docs; make the repo public | Repo, Docs | A8, R-09, R-10 | 6 | Written search results in the M9 issue; name final; repo public |
...
| M5 Keyboard | iOS contributor (M5-08…M5-12) ∥ Mac contributor (M5-03…M5-07) after M5-01/02 land | Interface is the message catalogue; `RecordingInjector` lets the Mac side test without a phone; `aircontrol-cli type/key` lets the iOS side be tested against the Mac without the iOS UI |
...
- `aircontrol-cli replay` gesture-trace format and runner (M4 optional)
...
**Per PR (human).** The PR template requires: spec section(s) touched; protocol change → `docs/protocol.md` CHANGELOG entry; storage schema change → migrator + `CHANGELOG` "Storage" entry; UI change → screenshot in light and dark; any file under `AirControlCrypto` or `.github/workflows` → CODEOWNER review.
...
cd "<path to your air-control checkout>"
git init -b main
mkdir -p Packages/AirControlKit/Sources/{AirControlProtocol,AirControlCrypto,AirControlFilters,AirControlCore,aircontrol-cli} \
         Packages/AirControlKit/Tests/{AirControlProtocolTests,AirControlCryptoTests,AirControlFiltersTests,AirControlCoreTests}/Vectors \
         apps/AirControl-iOS/{Sources/{App,Features,Services,Support},Resources,Tests,UITests} \
         apps/AirControl-Mac/{Sources/{App,Features,Services,Support},Resources,Tests,IntegrationTests} \
...
brew "create-dmg"
EOF
curl -sL https://opensource.org/license/mit -o /dev/null && printf 'MIT License\n\nCopyright (c) 2026 Air Control contributors\n...\n' > LICENSE   # paste the full MIT text

# 4. Package manifest (paste arch §2.2), placeholder sources and one test per target, then:
cd Packages/AirControlKit && swift build && swift test && cd ../..

# 5. XcodeGen specs (paste arch §2.4 and §2.5), then generate and build
xcodegen generate --spec apps/AirControl-iOS/project.yml --project apps/AirControl-iOS
xcodegen generate --spec apps/AirControl-Mac/project.yml --project apps/AirControl-Mac
xcodebuild -project apps/AirControl-iOS/AirControl.xcodeproj -scheme AirControl \
  -destination 'platform=iOS Simulator,name=iPhone 16' build CODE_SIGNING_ALLOWED=NO
xcodebuild -project apps/AirControl-Mac/AirControlHelper.xcodeproj -scheme AirControlHelper build
codesign -dv --verbose=2 ~/Library/Developer/Xcode/DerivedData/AirControlHelper-*/Build/Products/Debug/Air\ Mouse.app 2>&1 | grep -E 'Authority|runtime'
...
gh auth login
gh repo create air-control --private --source=. --push
...
Bundle identifiers stay `com.aircontrol.*` as the codename until M9-11; `BUNDLE_ID_SUFFIX` keeps each developer's debug helper distinct from a future release install (TCC and Launch Services both key on it).
...
| Windows / Linux host | v2 (community) | 15 d each | `AirControlProtocol` is the contract; host needs SendInput / uinput injectors |
...
| **M2 motion latency** | p50 ≤ 12 ms, p95 ≤ 20 ms, p99 ≤ 30 ms | Camera ground truth + `aircontrol-cli bench` + `xctrace` per spec §10.4, results committed to `docs/perf/`; the in-app Latency HUD gives every user the same numbers, and the `latency-report.yml` issue template asks for HUD p50/p95, network band, phone/Mac models | Maintainer at M4 gate and each release; community via issues |
