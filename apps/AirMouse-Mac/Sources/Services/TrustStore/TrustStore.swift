...
//
// Conforms to both `AirMouseCore.TrustStoreProtocol` (the shape `HostSession`/`PairingService`
...
// angles (`AirMouseCrypto.Fingerprint` vs. a `String` id), so this actor is the single adapter
...
/// Disambiguates `AirMouseCore.TrustedDeviceRecord` from the shell's own same-named type
/// (`App/ServiceProtocols.swift`) — see this file's header.
public typealias CoreTrustedDeviceRecord = AirMouseCore.TrustedDeviceRecord
...
public actor TrustStore: TrustStoreProtocol, TrustStoring {
    ...
    // MARK: - AirMouseCore.TrustStoreProtocol
    ...
    /// `AirMouseCrypto.PinningPolicy.hostDecision`'s `trustedFingerprints`/`pendingConnectionCount`
    ...
    private func deleteKeychainCertificate(fingerprint: Fingerprint) {
        let label = "AirMouse Trusted Client \(fingerprint.hexString)"
