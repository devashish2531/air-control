...
public enum HostFeature {
    ...
    /// drained to the JSON-lines file at `$AIRMOUSE_LOOPBACK_LOG` (one object per line: `kind`,
    ...
    /// `AIRMOUSE_TCP_PORT=<n>` / `AIRMOUSE_UDP_PORT=<n>` / `AIRMOUSE_PAIR_URL=<url>` to stdout (each
    ...
    public static func make(
        ...
        let loopbackLogURL: URL? = {
            guard loopback, let path = ProcessInfo.processInfo.environment["AIRMOUSE_LOOPBACK_LOG"] else { return nil }
