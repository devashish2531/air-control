// NWControlChannel — Network.framework implementation of `AirMouseCore.ControlChannel` (spec
// §3.4.1 framing lives in AirMouseCore; this only moves already-framed bytes over one accepted
...
/// `HostServer`) as an `AirMouseCore.ControlChannel`. `incoming` yields raw received byte chunks —
/// `AirMouseCore.HostSession` owns `FrameDecoder` and feeds these chunks into it itself (see that
...
public actor NWControlChannel: ControlChannel {
    ...
    /// with label `"EXPORTER-airmouse-pairing-v1"`, 32 bytes. Returns `nil` if TLS metadata isn't
