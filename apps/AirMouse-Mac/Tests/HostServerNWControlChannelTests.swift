...
struct HostServerNWControlChannelTests {
    ...
    func framingRoundTrip() async throws {
        let hostIdentity = try IdentityFactory.makeEphemeralIdentity(commonName: "AirMouse Host test")
        let clientIdentity = try IdentityFactory.makeEphemeralIdentity(commonName: "AirMouse Client test")
    ...
    func peerFingerprintExtraction() async throws {
        let hostIdentity = try IdentityFactory.makeEphemeralIdentity(commonName: "AirMouse Host test 2")
        let clientIdentity = try IdentityFactory.makeEphemeralIdentity(commonName: "AirMouse Client test 2")
        let (host, client, listener) = try await Self.makeConnectedPair(hostIdentity: hostIdentity, clientIdentity: clientIdentity)
        defer { listener.cancel() }
    ...
    func exporterSecretAgrees() async throws {
        let hostIdentity = try IdentityFactory.makeEphemeralIdentity(commonName: "AirMouse Host test 3")
        let clientIdentity = try IdentityFactory.makeEphemeralIdentity(commonName: "AirMouse Client test 3")
        let (host, client, listener) = try await Self.makeConnectedPair(hostIdentity: hostIdentity, clientIdentity: clientIdentity)
        defer { listener.cancel() }
