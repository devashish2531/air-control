...
@Suite struct MacroActionExecutorTests {
    ...
    @Test func launchAppWithUnknownBundleIDFails() async throws {
        ...
        let outcome = await executor.execute(.launchApp(bundleID: "com.airmouse.definitely-not-installed-\(UUID().uuidString)", activateIfRunning: false))
    ...
    @Test func openURLRejectsAnUnregisteredScheme() async throws {
        ...
        let url = URL(string: "airmouse-test-scheme-\(UUID().uuidString.prefix(8))://nowhere")!
