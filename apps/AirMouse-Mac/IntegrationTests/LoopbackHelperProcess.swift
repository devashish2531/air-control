...
// Locates and launches the built `AirMouse.app` helper in `--loopback` mode (contract with the
// networking/app-shell agents, `apps/AirMouse-Mac/Sources/Services/HostServer/HostServer.swift`'s
// `printLoopbackBanner()`): prints `AIRMOUSE_TCP_PORT=`/`AIRMOUSE_UDP_PORT=`/`AIRMOUSE_PAIR_URL=`
// lines to stdout and, once the app-shell agent wires it up, writes one JSON `PostedEvent` per line
// to the file named by the `AIRMOUSE_LOOPBACK_LOG` env var. Every step here fails soft (returns
...
final class LoopbackHelperProcess {
    ...
    /// `$BUILT_PRODUCTS_DIR/AirMouse.app/Contents/MacOS/AirMouse`, or — since this test bundle is
    ...
    static func locateExecutable() -> URL? {
        ...
        if let dir = env["BUILT_PRODUCTS_DIR"] {
            let candidate = URL(fileURLWithPath: dir).appendingPathComponent("AirMouse.app/Contents/MacOS/AirMouse")
    ...
    /// Launches `--loopback`, waits up to `timeout` seconds for `AIRMOUSE_PAIR_URL=` on stdout.
    ...
    static func launch(timeout: TimeInterval = 10) async -> LoopbackHelperProcess? {
        ...
        let logURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("airmouse-loopback-\(UUID().uuidString).jsonl")
        ...
        while Date() < deadline {
            if let line = helper.outputBox.value.split(separator: "\n").first(where: { $0.hasPrefix("AIRMOUSE_PAIR_URL=") }) {
                helper.pairURLString = String(line.dropFirst("AIRMOUSE_PAIR_URL=".count))
                return helper
            }
