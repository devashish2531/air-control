...
// this test target so `AirMouseHelperIntegrationTests` does not depend on `airmouse-cli` (a separate
// SwiftPM executable target this Xcode project does not, and should not, link against). Deliberately
// smaller than `airmouse-cli`'s `CLIControlChannel`/`CLIDatagramChannel` (Packages/AirMouseKit/Sources/
// airmouse-cli/Transport/) — same TLS recipe (arch §7.2: TLS 1.3, challenge block supplies the client
...
/// caller without a data race (mirrors `airmouse-cli`'s identical helper).
...
final class LoopbackControlChannel: ControlChannel, @unchecked Sendable {
    ...
    static func connect(host: String, port: Int, identity: SecIdentity, pinnedFingerprint: Fingerprint) async throws -> LoopbackControlChannel {
        ...
        let tlsQueue = DispatchQueue(label: "com.airmouse.integrationtests.tls")
