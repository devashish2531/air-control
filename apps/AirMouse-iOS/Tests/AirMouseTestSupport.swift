// Tests/AirMouseTestSupport.swift
// Fakes for `AirMouseViewModel` feature tests: a recording `HapticsService`,
// `ControlMessageSink` (Features/Touchpad, another agent's protocol — reused rather than
// duplicated, see `AirMouseViewModel.swift`), and `MotionEnqueuing` sink for scroll deltas.
...
func makeAirMouseViewModel(
    ...
    let suite = "AirMouseViewModelTests-\(UUID().uuidString)"
    let defaults = UserDefaults(suiteName: suite)!
    let userSettings = UserSettings(defaults: defaults)
    let localSettings = AirMouseLocalSettings(defaults: defaults)
