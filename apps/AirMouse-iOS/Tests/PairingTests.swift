...
@Suite struct PairingTests {
    ...
    @Test func rightSchemeMissingRequiredParamsIsRejected() async {
        let manager = makeManager()
        // `airmouse://pair` with no query at all — missing every required param.
        await manager.pair(urlString: "airmouse://pair")
        #expect(manager.pairingProgress == .failed(.pairingURLInvalid))
    }

    @Test func wrongHostInTheRightSchemeIsRejected() async {
        let manager = makeManager()
        await manager.pair(urlString: "airmouse://not-pair?v=1")
        #expect(manager.pairingProgress == .failed(.pairingURLInvalid))
    }
    ...
    @Test func oversizedURLIsRejected() async {
        ...
        await manager.pair(urlString: "airmouse://pair?v=1&id=AAAAAAAAAAAAAAAAAAAAAA&n=\(hugeName)&a=10.0.0.1&p=47800&fp=AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA&s=AAAAAAAAAAAAAAAAAAAAAA")
        #expect(manager.pairingProgress == .failed(.pairingURLInvalid))
    }
