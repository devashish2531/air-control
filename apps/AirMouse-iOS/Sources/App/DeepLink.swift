// App/DeepLink.swift
// Recognises `airmouse://pair?...` URLs (spec §4.1.2) so `RootTabView` can forward them to the
...
public enum DeepLink {
    /// `true` for any URL whose scheme is `airmouse` and host is `pair`, case-insensitively.
    ...
    public static func isPairingURL(_ url: URL) -> Bool {
        guard let scheme = url.scheme?.lowercased(), scheme == "airmouse" else { return false }
