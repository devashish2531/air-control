...
// `Application Support/AirMouse/`, a versioned envelope (`{ "schema": "<name>/<int>", ... }`),
...
public struct DocumentDescriptor<T: Codable & Sendable>: Sendable {
    /// Path relative to the store's root directory (`Application Support/AirMouse/`), e.g.
...
public actor DocumentStore {
    ...
    /// - Parameter rootDirectory: Defaults to `Application Support/AirMouse/` in the app's
    ...
    public init(rootDirectory: URL? = nil) {
        if let rootDirectory {
            ...
            self.rootDirectory = appSupport.appendingPathComponent("AirMouse", isDirectory: true)
        }
    }
