...
// A protocol wrapping the exact `AirMouseCore.ClientSession` calls `ConnectionManager` uses, so
...
// `actor` (not a protocol) in `AirMouseCore`, so this is this module's own minimal seam over it —
...
// behavior. `AirMouseProtocol.Text` is spelled out to avoid colliding with `SwiftUI.Text`.
