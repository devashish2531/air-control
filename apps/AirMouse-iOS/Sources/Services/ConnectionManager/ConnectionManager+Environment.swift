...
public enum ConnectionFeature {
    ...
    private static func loadOrCreateClientIdentity() -> GeneratedIdentity {
        ...
        if ProcessInfo.processInfo.environment["AIRMOUSE_EPHEMERAL_CLIENT_IDENTITY"] == "1" {
            Log.net.notice("AIRMOUSE_EPHEMERAL_CLIENT_IDENTITY=1: using a fresh ephemeral client identity, bypassing the Keychain")
            Self.identityTrace = "debug-ephemeral"
            return try! IdentityFactory.makeEphemeralIdentity(commonName: "AirMouse Client (debug ephemeral)")
        }
        #endif
