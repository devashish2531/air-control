// Features/AirMouse/AirMouseFeature.swift
// Factory that wires a `GyroEngine` + `AirMouseViewModel` from `AppEnvironment` (arch §3.2:
// "View models are constructed by the views from the environment and never construct services
// themselves" — this is that construction step for the Air Mouse tab).
//
...
// assignment) hides the Air Mouse tab whenever that's false — computed once per render, before
// this screen ever gets a chance to run and swap in the real `GyroEngine`. For the tab to appear
// on a real device, whoever constructs `AppEnvironment` in `AirMouseApp.swift` should pass a real
// `GyroEngine` at startup, e.g. `AppEnvironment(gyro: GyroEngine())`. Once that happens,
// `AirMouseFeature.make(environment:)` below detects and reuses that same instance rather than
...
public enum AirMouseFeature {
    @MainActor
    public static func make(environment: AppEnvironment) -> AirMouseScreen {
        AirMouseScreen(viewModel: makeViewModel(environment: environment))
    }
