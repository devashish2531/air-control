// Features/AirMouse/AirMouseUnsupportedView.swift
...
// real `GyroEngine` is wired into `AppEnvironment` — see `AirMouseFeature.swift`'s integration
// note).
import SwiftUI

struct AirMouseUnsupportedView: View {
    var body: some View {
        VStack(spacing: 12) {
            ...
            SwiftUI.Text("Air Mouse needs a gyroscope to turn your phone's motion into cursor movement.", comment: "Gyro tab unsupported-device explanation")
                .font(.subheadline)
                .foregroundStyle(.secondary)
                .multilineTextAlignment(.center)
        }
