...
// macro engine). This module owns no networking (CLAUDE.md: "Only `aircontrol-cli` and the two apps
...
//
// `MacroInvoke.id`/`Macro.id` are `UUID` on the wire model (AirControlProtocol/Macros/*), so
...
/// arrives for the matching `ref`. Reuses `MacroResultCode` (AirControlProtocol/Macros) — a `Macro`
...
public protocol RemoteCommandSink: AnyObject {
    ...
    /// `virtualKey` is one of the `VirtualKey.kVK_*` constants (AirControlProtocol/Keycodes).
