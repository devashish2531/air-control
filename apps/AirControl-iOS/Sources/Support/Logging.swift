...
// `com.aircontrol.app`. Never log keys, secrets, typed text, or full fingerprints (spec §7).

import os

/// Namespaced `Logger` instances, one per architectural area, so log filtering in Console.app
/// can isolate a subsystem quickly (`subsystem:com.aircontrol.app category:net`).
public enum Log {
    private static let subsystem = "com.aircontrol.app"
