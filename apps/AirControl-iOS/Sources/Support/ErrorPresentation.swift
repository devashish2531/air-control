...
//
// `AirControlCore.AirControlError` (arch §8 "Error taxonomy") is the eventual source of these values
...
// its `AirControlError` to this enum once `AirControlCore` lands.
...
public enum AppError: Sendable, Equatable {
    ...
    /// `AirControlCore.HostSessionStateMachine`) — the fix is simply to reconnect, not to pair again.
    case pairingAlreadyTrusted
    /// A peer message arrived that made no sense for the phase the session was in (spec decision:
    /// see `AirControlCore.CoreError.protocolMismatch`) — distinct from `.generic`'s bare wire code so
    ...
    public var presentation: ErrorPresentation {
        ...
        case .localNetworkDenied:
            return ErrorPresentation(
                ...
                message: String(localized: "Air Control can't see your Mac until you allow Local Network access in Settings."),
        ...
        case .noHostsFound:
            return ErrorPresentation(
                ...
                message: String(localized: "Is the Air Control helper running on your Mac? Are both devices on the same Wi-Fi? Is Local Network access allowed?"),
        ...
        case .hostUnreachable(let hostName):
            return ErrorPresentation(
                ...
                title: String(localized: "Couldn't reach \(hostName)"),
                message: String(localized: "Make sure your iPhone and Mac are on the same Wi-Fi and that Local Network access is allowed for Air Control in Settings › Privacy & Security › Local Network."),
        ...
        case .tlsVerificationFailed(let hostName):
            return ErrorPresentation(
                ...
                message: String(localized: "Its certificate didn't match what Air Control expected. If you reinstalled or reset the Mac helper, forget this Mac and pair again."),
        ...
        case .pairingURLInvalid:
            return ErrorPresentation(
                ...
                title: String(localized: "That's not an Air Control code"),
                message: String(localized: "Scan the QR shown by 'Pair new device' in the Air Control menu on your Mac."),
                actions: [.tryAgain])
        case .pairingVersionMismatch:
            return ErrorPresentation(
                id: "E-PAIR-VERSION", style: .alert,
                title: String(localized: "Update Air Control"),
        ...
        case .versionAppOutdated:
            return ErrorPresentation(
                ...
                title: String(localized: "Update the Air Control app"),
        ...
        case .versionHelperOutdated:
            return ErrorPresentation(
                ...
                message: String(localized: "Open Air Control on your Mac and choose Check for Updates, or run brew upgrade --cask air-control."),
                actions: [.ok])
        ...
        case .pausedOnMac:
            return ErrorPresentation(
                ...
                title: String(localized: "Paused on Mac — input is ignored until you resume it from the Air Control menu."),
                message: "",
                actions: [])
        case .accessibilityMissing:
            return ErrorPresentation(
                ...
                message: String(localized: "Open Air Control on the Mac and follow the setup to allow it to control the cursor."),
                actions: [])
        case .macroBlocked:
            return ErrorPresentation(
        ...
        case .gyroUnavailable:
            return ErrorPresentation(
                ...
                title: String(localized: "This device has no gyroscope, so Air Pointer mode isn't available."),
                message: "",
                actions: [])
        case .gyroCalibrating:
            return ErrorPresentation(
