...
// aircontrol-cli import Network) and no crypto (AirControlCrypto is a different layer) — sealing the
// 16-byte payload into the 44-byte AEAD datagram and handing it to `NWConnection`/
// `NWDatagramTransport` is the Connection agent's `ClientSession` (AirControlCore). This protocol is
...
/// job: `AirControlCrypto.seal` it into the 44-byte UDP datagram (§3.5.1) and send it — or, while in
