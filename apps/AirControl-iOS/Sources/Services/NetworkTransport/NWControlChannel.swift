...
// Client-side control (TCP/TLS) channel implementing `AirControlCore.ControlChannel` over
...
// `aircontrol-cli` and the two apps import Network.framework").
//
// `incoming` yields RAW received TCP byte chunks, *not* pre-decoded frames: reading
// `ClientSession.runReceiveLoop`/`handleIncomingControlData` (AirControlCore) shows it owns its own
...
/// Transport-level errors surfaced by this module's channels — distinct from `AirControlCore`'s own
...
/// One TLS 1.3 control connection to a host's `_aircontrol._tcp` endpoint, pinned to a known
...
public final class NWControlChannel: ControlChannel, @unchecked Sendable {
    ...
    public static func connect(
        ...
        let queue = DispatchQueue(label: "com.aircontrol.app.net.control")
    ...
    /// spec §3.2.3: `TLS-Exporter("EXPORTER-aircontrol-pairing-v1", 32 B)`, empty context, via
    ...
    /// project's `AIRCONTROL_WARNINGS_AS_ERRORS`. Values per `<Security/SecureTransport.h>`.
