...
// `_aircontrol._tcp`", "`includePeerToPeer = false`", "results debounced 100 ms", cross-talk guard
...
///
/// This is `NetworkTransport`'s own model — `AirControlCore`/`AirControlProtocol` know `TXTRecord` but
...
/// Browses `_aircontrol._tcp` and publishes the current result set as `DiscoveredHost` snapshots.
...
public final class BonjourBrowser: @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "com.aircontrol.app.net.browser")
