...
//   only while the Air Pointer tab... is visible and the app is active; stopped otherwise
...
//   `AirPointerViewModel`); recommend the Settings-owning agent add the missing fields so a single
...
public final class GyroEngine: GyroEngineProviding {
    ...
    public init(
        ...
        queue.name = "com.aircontrol.app.gyro"
    ...
    // MARK: - GyroEngineProviding (spec: tab appear/disappear per arch §3.2's AirControl feature row)
