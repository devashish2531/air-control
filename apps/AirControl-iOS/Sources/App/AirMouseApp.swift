...
// transitions (scenePhase) are forwarded by AirControlApp") is wired here as a thin `scenePhase`
...
private struct RootView: View {
    ...
    var body: some View {
        RootTabView()
            .task {
                #if DEBUG
                // Test hook: `AIRCONTROL_PAIR_URL` env (or `-pairURL <url>` argument) routes a pairing URL at
...
#if DEBUG
extension RootView {
    /// `AIRCONTROL_PAIR_URL=aircontrol://…` in the environment, or `-pairURL aircontrol://…` in the arguments.
    static func launchPairingURLString() -> String? {
        let info = ProcessInfo.processInfo
        if let env = info.environment["AIRCONTROL_PAIR_URL"], !env.isEmpty { return env }
