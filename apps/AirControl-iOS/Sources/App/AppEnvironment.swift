...
// DI container (arch §3.2 "Dependency injection"). Built once in `AirControlApp` and injected
...
public extension AppEnvironment {
    ...
    static func preview() -> AppEnvironment {
        AppEnvironment(
            ...
            documentStore: DocumentStore(rootDirectory: FileManager.default.temporaryDirectory.appendingPathComponent("AirControlPreview-\(UUID().uuidString)"))
        )
    }
}
...
/// environment's owned services to already exist, and `GyroEngine`/`AirPointerFeature` need the
...
public extension AppEnvironment {
    ...
    static func live() -> AppEnvironment {
        ...
        // Gyro (spec §4.3, FR-GY-012): real availability gates the Air Pointer tab in
        // `RootTabView`. Mirrors the settings `AirPointerFeature.makeViewModel` would use if it had
