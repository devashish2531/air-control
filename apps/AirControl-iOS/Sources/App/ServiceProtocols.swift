...
/// What the shell needs from the gyro engine: availability (to hide the Air Pointer tab per
/// FR-GY-012 / spec §4.1) and lifecycle hooks the AirControl feature calls on appear/disappear.
@MainActor
public protocol GyroEngineProviding: AnyObject {
    /// `CMMotionManager().isDeviceMotionAvailable` — the Air Pointer tab is hidden when false.
...
/// Forwards a scanned or deep-linked pairing URL (`aircontrol://pair?...`, spec §4.1.2) to the
