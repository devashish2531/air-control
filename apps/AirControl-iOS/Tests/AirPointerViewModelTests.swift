// Tests/AirPointerViewModelTests.swift
...
@MainActor
@Suite struct AirPointerViewModelTests {
    @Test func holdModeClutchFiresEngageThenReleaseHaptics() async {
        let haptics = FakeHapticsService()
        let viewModel = makeAirPointerViewModel(clutchMode: .hold, haptics: haptics)
    ...
    @Test func toggleModeClutchFiresEngageOnFirstTapAndReleaseOnSecond() async {
        let haptics = FakeHapticsService()
        let viewModel = makeAirPointerViewModel(clutchMode: .toggle, haptics: haptics)
    ...
    @Test func quickPrimaryPressSendsASingleTapClick() async {
        ...
        let viewModel = makeAirPointerViewModel(controlSink: controlSink)
    ...
    @Test func scrollDragReportsBeganEndedPhasesAndPerStepDeltas() async {
        ...
        let viewModel = makeAirPointerViewModel(controlSink: controlSink, motionSink: motionSink)
    ...
    @Test func recenterButtonTriggersEngineRecenterAndHaptic() {
        let haptics = FakeHapticsService()
        let viewModel = makeAirPointerViewModel(haptics: haptics)
