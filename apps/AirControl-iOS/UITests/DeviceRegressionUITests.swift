...
/// (the phone is already paired, so this is the normal cold-launch path — no AIRCONTROL_PAIR_URL
...
///   xcodebuild test-without-building ... -only-testing:AirControlUITests/DeviceRegressionUITests
final class DeviceRegressionUITests: XCTestCase {
    ...
    private func launchAndWaitForConnected(timeout: TimeInterval = 45) -> XCUIApplication {
        ...
        if let url = ProcessInfo.processInfo.environment["AIRCONTROL_PAIR_URL"], !url.isEmpty {
