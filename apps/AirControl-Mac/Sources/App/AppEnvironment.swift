...
public final class AppEnvironment {
    ...
    public static func live(launchArguments: LaunchArguments = .parse()) async -> AppEnvironment {
        if launchArguments.loopback, ProcessInfo.processInfo.environment["AIRCONTROL_DATA_DIR"] == nil {
            // Throwaway run: keep its trust records, macros and host id out of the real store.
            let temp = FileManager.default.temporaryDirectory
                .appendingPathComponent("AirControlHelper-loopback-\(ProcessInfo.processInfo.processIdentifier)", isDirectory: true)
            setenv("AIRCONTROL_DATA_DIR", temp.path, 1)
        }
    ...
    /// already bound into every SwiftUI scene (`AirControlHelperApp`'s `@State`) stays the one every
    ...
    /// `@Observable` properties the menu and windows read directly. Call once from `AirControlHelperApp.init`.
