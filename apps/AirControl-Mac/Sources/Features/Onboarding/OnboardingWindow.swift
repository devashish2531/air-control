    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            Label("Accessibility", systemImage: "accessibility")
                .font(.title2.bold())
            Text("Air Control needs Accessibility permission to move the cursor and type on your behalf. It never reads your screen or your keystrokes.")
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            Label("Firewall", systemImage: "lock.shield")
                .font(.title2.bold())
            Text("Your Mac's firewall is on. The first time a device connects, macOS may ask whether to allow Air Control to receive incoming connections — choose Allow.")
                .fixedSize(horizontal: false, vertical: true)
        }
    }
}
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) {
            Label("Pair", systemImage: "qrcode")
                .font(.title2.bold())
            Text("Scan this code with the Air Control app on your iPhone or iPad.")
            PairingContentView()
        }
    }
}
