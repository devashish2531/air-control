// ActivityTrackingControlChannel — a thin `AirControlCore.ControlChannel` decorator that records the
// last time any bytes arrived from the peer. Owned by the networking agent (assignment:
// "Services/SessionManager/").
//
// `AirControlCore.HostSession.checkTimeouts(now:lastHeartbeatAt:)` needs its caller to track "when did
...
// decorator is the only seam available to observe it without modifying `AirControlCore` (CLAUDE.md:
