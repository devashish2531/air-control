...
//
// Per accepted connection, builds one `AirControlCore.HostSession`, consumes its `events` stream and
...
// attached) — `AirControlCore.HostSession` has no dedicated event for that. An unauthenticated
...
public actor SessionManager {
    ...
    /// aircontrol-cli/integration-test agent).
    ...
    private func tick() async {
        ...
        for key in Array(sessions.keys) {
            ...
            // the race that made a fast pair-then-disconnect (e.g. `aircontrol-cli pair`, which closes
    ...
    /// `aircontrol-cli pair`) can never race a slower, once-a-second-polled write and lose the record.
    ...
    private func mapButton(_ button: MouseButton) -> AirControlProtocol.MouseButton { button }
