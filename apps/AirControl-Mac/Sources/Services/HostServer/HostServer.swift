...
// `AirControlCore.HostSession`. Conforms to the shell's `HostServing` (`App/ServiceProtocols.swift`).
...
public actor HostServer: HostServing {
    /// Overridable via `AIRCONTROL_IDENTITY_LABEL` purely so a throwaway, alternate-port copy of the
    ...
    private var identityLabel: String {
        ProcessInfo.processInfo.environment["AIRCONTROL_IDENTITY_LABEL"] ?? "AirControl Host Identity"
    }
    private let trustStore: TrustStore
    private let pairingService: PairingService
    private let sessionManager: SessionManager
    private let settings: HostServerSettings
    private let netQueue = DispatchQueue(label: "com.aircontrol.helper.net", qos: .userInteractive)
    ...
    private func loadOrCreateIdentity() async throws -> GeneratedIdentity {
        if settings.loopback || !settings.persistIdentity {
            ...
            return try IdentityFactory.makeEphemeralIdentity(
                commonName: "AirControl Host \(hostID.b64u)",
                label: identityLabel
            )
        }
    ...
    private func printLoopbackBanner() async {
        ...
        let udpPort = udpHub.boundPort ?? tcpPort
        print("AIRCONTROL_TCP_PORT=\(tcpPort)")
        print("AIRCONTROL_UDP_PORT=\(udpPort)")
        fflush(stdout)
        if let urlString = try? await openPairingWindow() {
            print("AIRCONTROL_PAIR_URL=\(urlString)")
            fflush(stdout)
        }
    }
}
...
public struct HostServerSettings: Sendable {
    ...
    /// Whether `start()` registers the Bonjour (`_aircontrol._tcp`) service in non-loopback mode.
