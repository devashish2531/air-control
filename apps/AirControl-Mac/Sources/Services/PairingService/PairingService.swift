...
//
// Owns the single `AirControlCore.PairingWindow` (value type) lifecycle: open/close, the 60 s
// secret-regeneration-while-open rule, and QR/manual-fallback URL composition. The actual
// proof verification (`pairProof`/`pairConfirm`) happens inside `AirControlCore.HostSession` itself
...
/// Disambiguates `AirControlCore.PairingWindow` from the SwiftUI `PairingWindow` view
/// (`Features/Pairing/PairingWindow.swift`, same app target) — see this file's header.
public typealias CorePairingWindow = AirControlCore.PairingWindow
