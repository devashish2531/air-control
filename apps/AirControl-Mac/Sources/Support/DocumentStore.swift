...
// "~/Library/Application Support/Air Control/", but arch §6.3's own table of concrete documents uses
// "~/Library/Application Support/AirControlHelper/..." for every host-side path. Arch §6.3 is authoritative
...
/// Codable JSON document store for `~/Library/Application Support/AirControlHelper/` (arch §6.3). One
...
public actor DocumentStore {
    ...
    public static var defaultBaseDirectory: URL {
        // `AIRCONTROL_DATA_DIR` redirects every document (trust store, macros, host id) to another folder.
        ...
        if let override = ProcessInfo.processInfo.environment["AIRCONTROL_DATA_DIR"], !override.isEmpty {
            return URL(fileURLWithPath: override, isDirectory: true)
        }
        return FileManager.default.homeDirectoryForCurrentUser
            .appendingPathComponent("Library/Application Support/AirControlHelper", isDirectory: true)
    }
