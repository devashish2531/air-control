...
// `AirControl.app --loopback`, pairs a `ClientSession` (built on `LoopbackClientTransport.swift`, not
// `aircontrol-cli`) against the printed `AIRCONTROL_PAIR_URL`, drives it through motion/click/text/media,
// and asserts against the `PostedEvent` JSON lines the helper's `--loopback` mode is contracted to
// write to `AIRCONTROL_LOOPBACK_LOG` (`Services/EventInjector/RecordingEventPoster.swift`'s
...
// this writing, nothing in `App/AppEnvironment.swift`/`AirControlHelperApp.swift` has been observed to
...
/// and `aircontrol-cli`'s identical file — this target cannot depend on either).
...
private enum LoggedEventFields {
    /// Parses one `AIRCONTROL_LOOPBACK_LOG` line into a loosely-typed dictionary — deliberately not a
...
private struct PairedSession {
    ...
    /// Launches the helper, waits for `AIRCONTROL_PAIR_URL`, and pairs. Returns `nil` (caller must
    ...
    static func establish() async -> PairedSession? {
        ...
        guard let identity = try? IdentityFactory.makeEphemeralIdentity(commonName: "AirControl Integration Test Client") else {
            helper.terminate()
            return nil
        }
        ...
        let session = ClientSession(
            ...
            device: Hello.Device(name: "AirControlHelperIntegrationTests", model: "test", os: "macOS", app: "1.0"),
            displayHz: 60
        )
...
struct LoopbackIntegrationTests {
    ...
    @Test func gyroSourceMotionSumsExactly() async throws {
        guard let paired = await PairedSession.establish() else {
            skip("helper not built, --loopback didn't print AIRCONTROL_PAIR_URL in time, or pairing failed")
            return
        }
        guard await paired.logPipelineIsLive() else {
            skip("AIRCONTROL_LOOPBACK_LOG never received a line — --loopback's EventPosting wiring isn't finished yet")
    ...
    @Test(.enabled(if: ProcessInfo.processInfo.environment["AIRCONTROL_PERF"] == "1"))
